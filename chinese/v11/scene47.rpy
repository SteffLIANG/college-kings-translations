# TODO: Translation updated at 2023-06-17 17:22

# game/v11/scene47.rpy:20
translate chinese v11_walking_back_57ae9944:

    # u "*Sighs*"
    u "*叹息*"

# game/v11/scene47.rpy:35
translate chinese v11_walking_back_0246002f:

    # u "(Wow... It's been so long since I've gotten a text, I forgot I even had this thing. *Chuckles*)"
    u "(哇……我已经很久没有收到短信了，我都忘了我还有这个东西。 *咯咯笑*)"

# game/v11/scene47.rpy:44
translate chinese v11s47_PhoneContinueJenny_88d710ce:

    # u "(I should check my phone.)"
    u "(我应该看一下我的手机。)"

# game/v11/scene47.rpy:50
translate chinese v11s47_PhoneContinueJenny_97b4ecf2:

    # u "(Well... That was unexpected.)"
    u "(嗯……这是出乎意料的。)"

# game/v11/scene47.rpy:65
translate chinese v11s47_PhoneContinueJenny_1da9fd43:

    # u "Hey man, look... Um. I know it's been a while since we last talked, but I just wanted to apologize for everything."
    u "嘿，伙计，听着……嗯。我知道距离我们上次谈话已经有一段时间了，但我只想为所有的事道歉。"

# game/v11/scene47.rpy:70
translate chinese v11s47_PhoneContinueJenny_c558181c:

    # u "I know we didn't exactly get off on the right foot since I was like, assuming things about you and that was really wrong of me. I shouldn't have gone about things the way that I did."
    u "我知道我们一开始就不是很好，因为我想，假设关于你的事情，那真的是我错了。我不该以那种方式行事。"

# game/v11/scene47.rpy:76
translate chinese v11s47_PhoneContinueJenny_f005d9bc:

    # u "You know... Since we last spoke I thought maybe, just maybe, I wasn't getting the \"bigger picture\"."
    u "你知道的……自从我们上次谈话以来，我想也许，只是也许，我没有了解\"大局\"。"

# game/v11/scene47.rpy:81
translate chinese v11s47_PhoneContinueJenny_c4b01a7f:

    # u "But the truth is, after speaking with MY friends, I know damn well I'm not a bad person like you try to make me out to be. You're just a stuck up asshole that doesn't know how to mind his own business."
    u "但事实是，在与我的朋友交谈后，我非常清楚地知道，我不是你试图把我塑造成的那种坏人。你只是个自命不凡的混蛋，不知道怎么管好自己的事。"

# game/v11/scene47.rpy:86
translate chinese v11s47_PhoneContinueJenny_55119281:

    # charli "Haha, [name]... I couldn't care less what you think about me, good or bad. And frankly, it doesn't matter what the girls think either, because they're too blinded by your bullshit to see the truth."
    charli "哈哈，[name]……我不在乎你怎么看我，是好是坏。坦率地说，女孩们怎么想也无关紧要，因为她们被你的屁话蒙蔽了双眼，看不到真相。"

# game/v11/scene47.rpy:91
translate chinese v11s47_PhoneContinueJenny_81b20669:

    # charli "You may think you're some kind of womanizer or player or whatever you consider to be manly in your small, delusional mind."
    charli "你可能认为自己是个花花公子，或者在你那小小的妄想症中，你认为自己是个男人。"

# game/v11/scene47.rpy:96
translate chinese v11s47_PhoneContinueJenny_4fcd9e15:

    # charli "Let me be very clear, you're a sad, disloyal dirtbag, desperate for female attention, so desperate that you don't care who you're hurting in the process."
    charli "让我把话说清楚，你是个可悲的，不忠的人渣，极度渴望得到女人的关注，以至于你根本不在乎你在这个过程中伤害了谁。"

# game/v11/scene47.rpy:101
translate chinese v11s47_PhoneContinueJenny_4e01f988:

    # charli "And your friends might not see that yet, but soon they will, I'll make sure of that."
    charli "你的朋友们可能还没发现，但很快他们就会了，我保证。"

# game/v11/scene47.rpy:106
translate chinese v11s47_PhoneContinueJenny_f4e83f60:

    # u "*Scoffs* Wow, you really are a giant piece of-"
    u "*嘲笑* 哇，你真是个超级……"

# game/v11/scene47.rpy:111
translate chinese v11s47_PhoneContinueJenny_c9d5fb3a:

    # no "AHHHH!!!!"
    no "啊！！！！"

translate chinese strings:

    # game/v11/scene47.rpy:60
    old "Tell him off"
    new "责备他"

